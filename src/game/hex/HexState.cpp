...
void HexGameState::init_zobrist(int seed)
{
	...
	for(int i = 0; i < N_PIECES; i++)
		for(int j = 0; j < size; j++)
		{
			...
			do
			{
				hash_strings[i][j] = rand();
				generate = false;
				for(int k = 0; k < i * size + j; k++)
					if(hash_strings[i][j] == hash_strings[k / size][k % size] && hash_strings[i][j] != 0)
					{
						generate = true;
						break;
					}
...
void HexGameState::PrintToFile(char* FilaName){
...
int HexGameState::UndoMove(int m) {
        moveCounter--;
